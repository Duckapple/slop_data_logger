      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-lg border border-slate-200 p-6">
        ...
        <ul className="mt-4 divide-y divide-slate-100">
          {entries.map((e) => (
            ...
            >
              <span className="text-sm text-slate-700">{e.description}</span>
              <span className="flex gap-1">
                {e.keys.map((k) => (
                  <kbd
                    key={k}
                    className="px-2 py-0.5 text-xs font-mono rounded border border-slate-200 bg-slate-50 text-slate-700"
                  >
                    {k}
                  </kbd>
                ))}
